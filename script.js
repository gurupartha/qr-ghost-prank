class QRGhostPrank {
    ...
    playWitchAudio() {
        try {
            // Try multiple methods to ensure witch audio plays
            const witchAudio1 = document.getElementById('witch-audio');
            if (witchAudio1) {
                witchAudio1.volume = 0.9;
                witchAudio1.currentTime = 0;
                witchAudio1.play().then(() => {
                    console.log('🧙‍♀️ Witch audio playing from preloaded element!');
                }).catch(() => {
            ...
            console.warn('Witch audio error:', error);
            this.playWitchAudioFallback();
        }
    }
    ...
    playWitchAudioFallback() {
        try {
            const witchAudio = new Audio('witch-scream.mp3');
            witchAudio.volume = 0.9;
            witchAudio.play().then(() => {
                console.log('🧙‍♀️ Witch audio playing from new Audio()!');
            }).catch(() => {
                console.log('🎃 Witch audio blocked, using synthetic sounds only');
            });
        } catch (error) {
